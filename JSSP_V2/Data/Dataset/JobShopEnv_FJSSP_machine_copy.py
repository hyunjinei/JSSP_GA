...
class JobShopEnv_FJSSP:
    def __init__(self, process_times, machine_sequence, solutions=None, analysis_values=None):
        ...
        self.multi_agent_graph = MultiAgentGraph(machines=list(range(self.n_machines)), jobs=list(range(self.n_jobs)))
        self.reset()
    ...
    def step(self, job, op, machine):
        ...
        step_reward = 1 if self.previous_idle_time is not None and current_idle_time == self.previous_idle_time else -2
    ...
    def update_graph(self, job, op, machine):
        ...
        # Update the MultiAgentGraph with the new state
        self.multi_agent_graph.update_graph(self.get_state(), op, machine)
