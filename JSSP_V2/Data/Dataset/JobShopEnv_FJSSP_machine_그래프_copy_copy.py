...
class JobShopEnv_FJSSP:
    ...
    def _init_graph(self):
        graph = {
            ...
            'jobs': {f'J_{j}': {'parents': set(), 'children': set(), 'current_op': 0, 'current_machine': {m: 0 for m in range(self.n_machines)}} for j in range(self.n_jobs)}
        }
    ...
    def get_state(self):
        ...
        for job in range(self.n_jobs):
            job_info = {
                'current_machine': {m: 0 for m in range(self.n_machines)},  # 초기화 시 각 기계에 대한 카운트를 0으로 설정
    ...
    def update_graph(self, job, op, machine):
        ...
        self.graph['jobs'][job_key]['current_machine'][machine] += 1
