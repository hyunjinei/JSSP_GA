...
class JobShopEnv_JSSP:
    def __init__(self, process_times, machine_sequence, solutions=None, analysis_values=None):
        print("Initializing JobShopEnv_JSSP")
        self.n_jobs = len(machine_sequence)
        self.n_machines = max(machine for job_seq in machine_sequence for machine in job_seq) + 1
    ...
    def _init_graph(self):
        ...
        for job in range(self.n_jobs):
            for op, machine in enumerate(self.machine_sequence[job]):
                if op > 0:
                    previous_machines = [f'M_{self.machine_sequence[job][op-1]}']
                ...
                if op < len(self.machine_sequence[job]) - 1:
                    next_machines = [f'M_{self.machine_sequence[job][op+1]}']
    ...
    def _init_machine_view(self):
        ...
        for job in range(self.n_jobs):
            for op, machine in enumerate(self.machine_sequence[job]):
                time = self.process_times[job][op][1]
    ...
    def get_state(self):
        ...
        for job in range(self.n_jobs):
            ...
            if current_op < len(self.machine_sequence[job]):
                for op, machine in enumerate(self.machine_sequence[job]):
                    if op == current_op:
                        job_info['waiting_machines'].add(machine)
                    elif op > current_op:
                        job_info['routing_machines'].add(machine)
        ...
        for job in range(self.n_jobs):
            ...
            if current_op > 0:
                self.graph['jobs'][job_key]['parents'] = {f'M_{self.machine_sequence[job][current_op-1]}'}
            if current_op < len(self.machine_sequence[job]) - 1:
                self.graph['jobs'][job_key]['children'] = {f'M_{self.machine_sequence[job][current_op+1]}'}
    ...
    def calculate_lowerbound(self):
        ...
        for job in range(self.n_jobs):
            ...
            for machine in remaining_operations:
                # 각 작업의 남은 각 작업에 대해 가능한 최소 처리 시간을 선택
                duration = next(duration for m, duration in self.process_times[job] if m == machine)
                lowerbound += duration
    ...
    def calculate_remaining_job_time(self):
        ...
        for job in range(self.n_jobs):
            ...
            for machine in remaining_operations:
                remaining_duration += next(duration for m, duration in self.process_times[job] if m == machine)
    ...
    def step(self, job, op, machine):
        ...
        if machine != machine_options:
            return self.state, -100, False  # 유효하지 않은 기계 선택에 대한 페널티

        processing_time = next(t for m, t in self.process_times[job] if m == machine)
    ...
    def update_graph(self, job, op, machine):
        ...
        if op > 0:
            self.graph['jobs'][job_key]['parents'] = {f'M_{self.machine_sequence[job][op-1]}'}
        if op < len(self.machine_sequence[job]) - 1:
            self.graph['jobs'][job_key]['children'] = {f'M_{self.machine_sequence[job][op+1]}'}
...
def calculate_early_start_bonus(agent_actions, env):
    ...
    for job, op, machine in agent_actions:
        processing_time = next((t for m, t in env.process_times[job] if m == machine), None)
...
def calculate_idle_time(agent_actions, env):
    ...
    for job, op, machine in agent_actions:
        ...
        duration = next((t for m, t in env.process_times[job] if m == machine), None)
        if duration is None:
