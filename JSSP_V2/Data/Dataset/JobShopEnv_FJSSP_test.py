...
class JobShopEnv_FJSSP:
    ...
    def get_state(self):
        ...
        return {
            ...
            'machine_available_time': tuple(self.machine_available_time),
    ...
    def step(self, selected_machine=None):
        # 기계별로 가능한 작업들을 가져옵니다.
        machine_jobs = {m: [] for m in range(self.n_machines)}
        for job in range(self.n_jobs):
            for op, machines in enumerate(self.machine_sequence[job]):
                if self.job_op_status[job][op]:
                    for machine, time in machines:
                        machine_jobs[machine].append((job, op, time))

        # 선택된 기계가 없을 경우, machine agent를 통해 기계를 선택합니다.
        if selected_machine is None:
            valid_machines = [m for m, jobs in machine_jobs.items() if jobs]
            if not valid_machines:
                raise Exception("No valid machines available for current operations.")
            selected_machine = random.choice(valid_machines)

        # 선택된 기계에서 가능한 작업 중 가장 앞의 Operation을 선택합니다.
        available_jobs = sorted(machine_jobs[selected_machine], key=lambda x: x[1])  # Operation 순서로 정렬
        job, op, processing_time = available_jobs[0]

        start_time = max(self.current_time, self.machine_available_time[selected_machine])
        end_time = start_time + processing_time

        self.machine_available_time[selected_machine] = end_time
        self.current_time = calculate_current_time(self.agent_actions + [(job, op, selected_machine)], self)
        self.machine_available_time = calculate_machine_available_time(self.agent_actions + [(job, op, selected_machine)], self)
        ...
        if self.machine_start_times[selected_machine] == -1:
            self.machine_start_times[selected_machine] = start_time

        self.agent_actions.append((job, op, selected_machine))
