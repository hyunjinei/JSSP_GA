...
class JobShopEnv_FJSSP:
    ...
    def get_state(self):
        ...
        # machine_available_time을 사전으로 변환
        ...
        state = (self.current_time, 
                 ...
                 tuple(machine_available_time_dict.values()),  # 사전의 값들을 튜플로 변환하여 사용
                 tuple(um_t),
                 tuple(qj_t),
                 tuple(pr_t))
                 #tuple(rj_t))
    ...
    def calculate_episode_rewards(self, is_pretrain=False):
        ...
        # reward_machine += early_start_bonus * 10  # 초기 보상 크기는 필요에 따라 조정
...
def calculate_early_start_bonus(agent_actions, env):
    ...
    if early_start_bonus == env.n_machines:
        early_start_bonus += 5  # 모든 머신이 0번째 시간에 시작했을 때 추가 보너스
