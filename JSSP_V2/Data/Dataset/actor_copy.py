...
class PtrNet1(nn.Module):
    def __init__(self, params):
        ...
        if torch.cuda.is_available():
            ...
            self.Vec2 = nn.Parameter(torch.cuda.FloatFloatTensor(params["n_hidden"]))
        else:
    ...
    def forward(self, x, device, y=None):
        ...
        embed_enc_inputs = self.Embedding(x.view(-1, input_dim)).view(batch, seq_len, -1)
        ...
        for i in range(seq_len):
            ...
            log_ps.append(log_p)
            mask = mask.scatter_(1, next_block.unsqueeze(1), 0)  # Update mask
