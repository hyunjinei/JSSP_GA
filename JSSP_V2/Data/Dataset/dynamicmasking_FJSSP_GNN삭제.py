...
class DDQNAgent:
    ...
    def replay(self):
        ...
        # actions 처리를 수정
        actions = [action if isinstance(action, (int, np.integer)) else next(iter(action.values())) for action in actions]
...
class MultiAgentSystem:
    ...
    def remember(self, agent_id, state, action, reward, next_state, done):
        self.agents[agent_id].remember(state, {agent_id: action}, reward, next_state, done)
        self.actual_actions[agent_id] = action  # 실제 액션을 저장
    ...
    for i, dataset in enumerate(datasets):
        ...
        for episode in range(num_episodes_per_dataset):
            ...
            while not done:
                        ...
                        if machine_id in available_machines:
                            for job, op, machine in valid_actions:
                                if machine == machine_id:
                                    action = multi_agent_system.agents[agent_id].act(state_features[agent_id])
                                    machine_actions[(job, op, machine)] = action
                ...
                multi_agent_system.remember(selected_machine_agent_id, state_features[selected_machine_agent_id], {selected_machine_agent_id: machine_actions[best_action]}, step_reward, next_state_features[selected_machine_agent_id], done)
