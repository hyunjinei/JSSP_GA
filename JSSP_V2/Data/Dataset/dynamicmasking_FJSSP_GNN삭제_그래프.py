...
class DDQNAgent:
    ...
    def replay(self):
        ...
        actions = [self.encode_action(action) for action in actions]
    ...
    def encode_action(self, action):
        # (job, operation) 튜플을 단일 정수로 인코딩
        if isinstance(action, tuple) and len(action) == 2:
            return action[0] * self.num_operations + action[1]
        return action  # 이미 인코딩된 경우
