...
class MultiAgentSystem:
    ...
    def replay(self):
        for agent_id, agent in self.agents.items():
            # print(f"Replay called for agent {agent_id}")
            if len(agent.memory) >= agent.replay_start_size:
                agent.replay()
            # else:
            #     print(f"Not enough memory to replay: {len(agent.memory)}/{agent.replay_start_size}")
    ...
    def end_episode(self, state_features, reward_task, reward_machine, done):
        ...
        for agent_id, agent in self.agents.items():
            agent.remember(state_features[agent_id], {agent_id: 0}, reward_task if agent_id.startswith('J_') else reward_machine, state_features[agent_id], done)
            agent.update_epsilon(self.global_episode_count)
        # if self.global_episode_count % 1 == 0:
        #     print(f'Episode {self.global_episode_count}, Epsilon: {self.agents[agent_id].epsilon:.4f}')
        if self.memory_size() >= self.replay_start_size:
            self.replay()

class DDQNAgent:
    def __init__(self, input_dim, action_size, update_target_frequency=32, replay_start_size=100, batch_size=32):
    ...
    def replay(self):
        ...
        actions = torch.LongTensor([action for action_dict in actions for action in action_dict.values()])  # 수정된 부분
...
def train_individual_models(datasets, num_episodes_per_dataset):
    for i, dataset in enumerate(datasets):
        ...
        for episode in range(num_episodes_per_dataset):
            state = env.reset()
            print(f"Episode {episode+1}/{num_episodes_per_dataset} started.")  # 에피소드 시작 알림
            done = False
            episode_reward = 0
            step_count = 0
            ...
            while not done:
                ...
                # Machine Agent만을 사용하여 작업 할당
                machine_actions = {agent_id: multi_agent_system.agents[agent_id].act(state_features[agent_id])
                                   for agent_id in multi_agent_system.agents if agent_id.startswith('M_')}
                
                selected_machine_agent_id = max(machine_actions, key=machine_actions.get)
                selected_machine_action = machine_actions[selected_machine_agent_id]

                selected_machine = int(selected_machine_agent_id.split('_')[1])
                selected_job = selected_machine_action // env.n_machines
                selected_op = selected_machine_action % env.n_machines

                # 선택된 에이전트와 할당된 작업을 출력
                print(f"Step {step_count}: Selected Machine Agent: {selected_machine_agent_id}, Job: {selected_job}, Operation: {selected_op}")

                valid_actions = env.get_valid_actions()
                if (selected_job, selected_op, selected_machine) not in valid_actions:
                    print(f"Invalid action: Job {selected_job}, Operation {selected_op}, Machine {selected_machine}")
                    continue
                ...
                multi_agent_system.remember(selected_machine_agent_id, state_features[selected_machine_agent_id], {selected_machine_agent_id: selected_machine_action}, step_reward, next_state_features[selected_machine_agent_id], done)
...
def predict(multi_agent_system, env, test_dataset, num_predictions=1, max_steps=100000000):
    ...
    while valid_solution_count < num_predictions and step_count < max_steps:
        ...
        while not done and step_count < max_steps:
            ...
            job_actions = {agent_id: multi_agent_system.agents[agent_id].act(state_features[agent_id])
                           for agent_id in multi_agent_system.agents if agent_id.startswith('J_')}
            best_job_agent_id = max(job_actions, key=job_actions.get)
            selected_job_action = job_actions[best_job_agent_id]

            selected_job = int(best_job_agent_id.split('_')[1])
            selected_op = selected_job_action % env.n_machines

            possible_machines = [m for m, _ in env.machine_sequence[selected_job][selected_op]]

            machine_actions = {agent_id: multi_agent_system.agents[agent_id].act(state_features[agent_id])
                               for agent_id in multi_agent_system.agents 
                               if agent_id.startswith('M_') and int(agent_id.split('_')[1]) in possible_machines}
            
            best_machine_agent_id = max(machine_actions, key=machine_actions.get)
            selected_machine = int(best_machine_agent_id.split('_')[1])
