...
class DDQNAgent:
    def __init__(self, input_dim, action_size, update_target_frequency=10, replay_start_size=100, batch_size=64, temperature=1.0, **kwargs):
    ...
    def act(self, state, valid_operations):
        ...
        valid_q_values = torch.tensor([q_values[i].item() for i, op in enumerate(valid_operations)])
        probabilities = F.softmax(valid_q_values / self.temperature, dim=0)
        
        # NumPy의 choice 함수는 확률 분포에 따라 선택을 합니다
        chosen_index = np.random.choice(len(valid_operations), p=probabilities.detach().numpy())
        return valid_operations[chosen_index]

    def update_temperature(self, factor=0.99):
        # 온도를 점진적으로 낮춥니다 (선택적)
...
class SupervisorAgent(DDQNAgent):
    ...
    def select_machine(self, available_machines, state_features):
        ...
        machine_values = torch.tensor(machine_values)
        probabilities = F.softmax(machine_values / self.temperature, dim=0)
        
        chosen_index = np.random.choice(len(available_machines), p=probabilities.detach().numpy())
        return available_machines[chosen_index]
...
class MultiAgentSystem:
    ...
    def end_episode(self, state_features, reward_task, reward_machine, done):
        ...
        for agent_id, agent in self.agents.items():
            ...
            agent.update_temperature()  # temperature 업데이트
        ...
        self.supervisor.update_temperature()  # temperature 업데이트
