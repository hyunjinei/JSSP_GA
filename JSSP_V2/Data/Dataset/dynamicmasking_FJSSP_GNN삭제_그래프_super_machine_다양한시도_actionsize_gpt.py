...
def featurize_action(action, env):
    ...
    job_progress = env.job_completion[job] / len(env.machine_sequence[job])
    op_progress = op / len(env.machine_sequence[job])
    
    # op_duration을 계산할 때 StopIteration 예외를 방지하기 위해 기본값을 설정합니다.
    op_duration = next((t for m, t in env.machine_sequence[job][op] if m == machine), 0)
    op_duration_normalized = op_duration / max(t for _, t in env.machine_sequence[job][op])
    
    return np.array([
        job / env.n_jobs,
        job_progress,
        op / len(env.machine_sequence[job]),
        op_progress,
        op_duration_normalized,
        machine / env.n_machines,
    ])

class DuelingDQN(nn.Module):
    def __init__(self, input_dim, action_feature_dim, action_size):
        ...
        self.advantage = nn.Sequential(
            nn.Linear(128 + action_feature_dim, 128),
            nn.GELU(),
    ...
    def forward(self, x, action_feature):
        feature = self.feature(x)
        combined_feature = torch.cat([feature, action_feature], dim=-1)
        advantage = self.advantage(combined_feature)
        value = self.value(feature)
        return value + advantage - advantage.mean(dim=-1, keepdim=True)


class DDQNAgent:
    def __init__(self, input_dim, action_feature_dim, action_size, update_target_frequency=80, replay_start_size=100, batch_size=80):
        ...
        self.model = DuelingDQN(input_dim, action_feature_dim, action_size)
        self.target_model = DuelingDQN(input_dim, action_feature_dim, action_size)
    ...
    def act(self, state, valid_operations, env):
        ...
        for op in valid_operations:
            action_feature = torch.FloatTensor(featurize_action(op, env)).unsqueeze(0)
            q_value = self.model(state, action_feature).item()
    ...
    def replay(self):
        ...
        actions = torch.LongTensor(np.array([each[1] for each in minibatch]))
        ...
        for state, action in zip(states, actions):
            action_feature = torch.FloatTensor(featurize_action(action, env)).unsqueeze(0)
            q_value = self.model(state.unsqueeze(0), action_feature).squeeze(0)
            q_values.append(q_value)
        ...
        for next_state, action in zip(next_states, actions):
            action_feature = torch.FloatTensor(featurize_action(action, env)).unsqueeze(0)
            q_value = self.model(next_state.unsqueeze(0), action_feature).squeeze(0)
        ...
        for next_state, action in zip(next_states, actions):
            action_feature = torch.FloatTensor(featurize_action(action, env)).unsqueeze(0)
            q_value = self.target_model(next_state.unsqueeze(0), action_feature).squeeze(0)
            next_q_state_values.append(q_value)
        next_q_state_values = torch.stack(next_q_state_values)
...
class SupervisorAgent(DDQNAgent):
    def __init__(self, input_dim, action_size, action_feature_dim, update_target_frequency=80, replay_start_size=100, batch_size=80):
        super().__init__(input_dim, action_size, action_feature_dim, update_target_frequency, replay_start_size, batch_size)
    ...
    def select_machine(self, available_machines, state_features, env):
        ...
        best_machine_index = int(np.argmax(q_values))
        
        return available_machines[best_machine_index]
class MultiAgentSystem:
    def __init__(self, machines, jobs, state_size, action_size, action_feature_dim):
    ...
    def initialize_agents(self, sample_state):
        ...
        self.agents = {f'M_{machine}': DDQNAgent(input_dim, self.action_size, self.action_feature_dim) for machine in self.machines}
        self.agents.update({f'J_{job}': DDQNAgent(input_dim, self.action_size, self.action_feature_dim) for job in self.jobs})
        self.supervisor = SupervisorAgent(input_dim, self.action_size, self.action_feature_dim)
        self.replay_start_size = 100
        self.global_episode_count = 0
...
def train_individual_models(datasets, num_episodes_per_dataset):
    for i, dataset in enumerate(datasets):
        ...
        action_feature_dim = 6  # featurize_action 함수에서 반환하는 배열의 크기
        multi_agent_system = MultiAgentSystem(machines=range(data.n_machine), jobs=range(data.n_job), state_size=None, action_size=1, action_feature_dim=action_feature_dim)
        ...
        for episode in range(num_episodes_per_dataset):
            ...
            while not done:
                ...
                if valid_operations:
                    ...
                    action_features = torch.FloatTensor(featurize_action(action, env)).unsqueeze(0)
                ...
                multi_agent_system.supervisor.remember(state_features['supervisor'], best_action, step_reward, next_state_features['supervisor'], done)
    ...
    while valid_solution_count < num_predictions and step_count < max_steps:
        ...
        while not done and step_count < max_steps:
            ...
            if valid_operations:
                ...
                machine_actions[action] = multi_agent_system.agents[selected_machine_agent_id].model(
                    torch.FloatTensor(state_features[selected_machine_agent_id]).unsqueeze(0),
                    torch.FloatFloatTensor(featurize_action(action, env)).unsqueeze(0)
                ).item()
