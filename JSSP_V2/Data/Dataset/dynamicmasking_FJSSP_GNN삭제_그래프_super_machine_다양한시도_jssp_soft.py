...
class DDQNAgent:
    ...
    def remember(self, state, action, reward, next_state, done):
        ...
        # action 값을 클리핑
        action = int(max(0, min(action, self.action_size - 1)))
        ...
        next_q_value = next_q_state_values[0][torch.argmax(next_q_values, 1)].item()
        td_error = abs(reward + self.gamma * next_q_value * (1 - done) - current_q_value)
        
        # 우선순위 계산
        priority = (td_error + self.PER_e) ** self.PER_a
    ...
    def act(self, state, valid_operations):
        ...
        # 소프트맥스 확률 계산
        exp_q_values = np.exp(target_q_values.detach().numpy()[valid_q_indices])
        probabilities = exp_q_values / np.sum(exp_q_values)
        
        selected_index = np.random.choice(valid_q_indices, p=probabilities)
        
        return valid_operations[selected_index]
...
class SupervisorAgent(DDQNAgent):
    ...
    def select_machine(self, available_machines, state_features):
        ...
        best_actions = torch.argmax(current_q_values).item()

        # 타겟 네트워크로 선택된 행동의 Q 값 평가
        target_q_values = self.target_model(state).squeeze(0)
        valid_q_values = {machine: target_q_values[machine].item() for machine in available_machines}
        
        # 소프트맥스 확률 계산
        exp_q_values = np.exp(list(valid_q_values.values()))
        probabilities = exp_q_values / np.sum(exp_q_values)
        
        selected_machine = np.random.choice(list(valid_q_values.keys()), p=probabilities)
...
class MultiAgentSystem:
    ...
    def initialize_agents(self, sample_state):
        ...
        self.supervisor = SupervisorAgent(input_dim, self.action_size)
        self.replay_start_size = 100
        self.global_episode_count = 0
    ...
    def replay(self):
        for agent_id, agent in self.agents.items():
            ...
            # if len(agent.memory.data) >= agent.replay_start_size:
                # agent.replay()
                # print(f"Machine agent {agent_id} replay called")  # Add this line
        self.supervisor.replay()
        # print("Supervisor replay called")  # Already present line
    ...
    def end_episode(self, state_features, reward_task, reward_machine, done, is_training=True):
        if is_training:
        ...
        # 평가 모드에서는 epsilon 값 변경하지 않고, memory에 저장하지 않음
        ...
        for episode in range(num_episodes_per_dataset):
            ...
            while not done:
                ...
                if done:
                    ...
                    # reward_task += episode_reward * 10
                    # reward_machine += episode_reward * 10
            ...
            multi_agent_system.end_episode(state_features, reward_task, reward_machine, done, is_training=True)
...
def predict(multi_agent_system, env, test_dataset, num_predictions=1, max_steps=100000000):
    ...
    while valid_solution_count < num_predictions and step_count < max_steps:
        ...
        while not done and step_count < max_steps:
            ...
            duration = next(t for m, t in env.process_times[selected_job] if m == selected_machine)
        ...
        if done:
            ...
            # reward_task += episode_reward * 10
            # reward_machine += episode_reward * 10
            multi_agent_system.end_episode(state_features, reward_task, reward_machine, done, is_training=False)
