...
from JobShopEnv_FJSSP_machine_그래프_copy_jssp import JobShopEnv_JSSP
...
class DDQNAgent:
    def __init__(self, input_dim, action_size, update_target_frequency=120, replay_start_size=300, batch_size=60):
    ...
    def act(self, state, valid_operations):
        ...
        if np.random.rand() <= self.epsilon:
            # 소프트 엡실론 그리디: 소프트맥스 분포 사용
            softmax_probs = F.softmax(valid_q_values / self.temperature, dim=0).detach().numpy()  # detach() 추가
            selected_index = np.random.choice(len(valid_q_indices), p=softmax_probs)
        else:
            selected_index = torch.argmax(valid_q_values).item()
        
        selected_action = valid_operations[selected_index]
...
class DDQNAgent_super:
    ...
    def select_machine(self, available_machines, state_features):
        ...
        for machine in available_machines:
            if machine == current_q_values.argmax().item():
                q_value = target_q_values[machine].item()
            else:
                q_value = current_q_values[machine].item()
            valid_q_values[machine] = q_value
        
        if np.random.rand() <= self.epsilon:
            # 소프트 엡실론 그리디: 소프트맥스 분포 사용
            q_values_list = [valid_q_values[m] for m in available_machines]
            softmax_probs = F.softmax(torch.tensor(q_values_list) / self.temperature, dim=0).detach().numpy()  # detach() 추가
            selected_machine = np.random.choice(available_machines, p=softmax_probs)
        else:
...
class MultiAgentSystem:
    ...
    def calculate_input_dim(self, state):
        machine_features_dim = len(self.graph.get_machine_features(f'M_0'))
        env_state_dim = (
            1 +  # current_time
        ...
        print(f"Calculated input dimension for machine agents: {total_dim}")  # 디버깅
        return total_dim

    def calculate_supervisor_input_dim(self, state):
        machine_features = sum(len(self.graph.get_machine_features(f'M_{i}')) for i in self.machines)
        job_features = sum(len(self.graph.get_job_features(f'J_{j}')) for j in self.jobs)
        env_state_length = (
            1 +  # current_time
            len(state['job_completion']) +
            len(state['machine_available_time']) +
        ...
        print(f"Calculated input dimension for supervisor agent: {total_dim}")  # 디버깅
        return total_dim
    ...
    def act(self, state, valid_actions):
        ...
        machine_state = np.concatenate([
            np.array(list(machine_features.values()), dtype=np.float32).flatten(),
    ...
    def get_supervisor_features(self, state):
        machine_features = [feature for m in self.machines for feature in self.graph.get_machine_features(f'M_{m}').values()]
        job_features = [feature for j in self.jobs for feature in self.graph.get_job_features(f'J_{j}').values()]
        env_features = [
            state['current_time'],
            *state['job_completion'],
            *state['machine_available_time'],
        ...
        print(f"Supervisor features shape: {len(features)}")  # 디버깅
        return features
    ...
    def end_episode(self, state, reward_task, reward_machine, done, is_training=True):
        if is_training:
            ...
            for agent_id in self.agents:
                if agent_id.startswith('M_'):
                    ...
                    machine_features = np.array(list(machine_features.values()))
                    ...
                    job_features = np.array(list(job_features.values()))
            ...
            for agent_id, agent in self.agents.items():
                ...
                if agent_id in state_features:  # state_features에 해당 agent_id가 있는지 확인
        ...
        print(f"Episode {self.global_episode_count} ended. Reward task: {reward_task}, Reward machine: {reward_machine}")
...
def train_individual_models(datasets, num_episodes_per_dataset):
    ...
    for i, dataset in enumerate(datasets):
        ...
        for job in data.op_data:
            job_process_times = []
            job_machine_sequence = []
            for op in job:
                m, t = op
                job_process_times.append((m, t))
        ...
        for episode in range(num_episodes_per_dataset):
            ...
            multi_agent_system.end_episode(state, reward_task, reward_machine, done, is_training=True)
    ...
    for agent_id in multi_agent_system.agents.keys():
        if agent_id.startswith('M_'):
            agent_save_paths[agent_id] = f"{agent_id}_jssp.pth"
        elif agent_id.startswith('J_'):
            agent_save_paths[agent_id] = f"{agent_id}_jssp.pth"
    agent_save_paths['supervisor'] = "supervisor_jssp.pth"
...
def predict(multi_agent_system, env, test_dataset, num_predictions=1, max_steps=100000000):
    ...
    for agent_id in multi_agent_system.agents.keys():
        if agent_id.startswith('M_'):
            agent_load_paths[agent_id] = f"{agent_id}_jssp.pth"
        # elif agent_id.startswith('J_'):
        #     agent_load_paths[agent_id] = f"{agent_id}_jssp.pth"
    agent_load_paths['supervisor'] = "supervisor_jssp.pth"
    ...
    while valid_solution_count < num_predictions and step_count < max_steps:
        ...
        while not done and step_count < max_steps:
            ...
            multi_agent_system.graph.update_graph(next_state)
            print(f"Process times structure: {env.process_times}")
            print(f"Selected job: {selected_job}, Selected op: {selected_op}")
            print(f"Process time: {env.process_times[selected_job][selected_op]}")

            machine, duration = env.process_times[selected_job][selected_op]
            if machine != selected_machine:
                print(f"Warning: Selected machine ({selected_machine}) does not match the machine in process_times ({machine})")
...
#abz5
    ...
    # 수정된 부분: process_times와 machine_sequence를 설정
    process_times = []
    machine_sequence = []
    
    for job in test_dataset.op_data:
        job_process_times = []
        job_machine_sequence = []
        for op in job:
            m, t = op
