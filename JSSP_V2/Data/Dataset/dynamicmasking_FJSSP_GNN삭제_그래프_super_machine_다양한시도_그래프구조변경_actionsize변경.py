...
class MultiAgentGraph:
    ...
    def get_machine_features(self, machine_id):
        ...
        features = (
            [1 if f'J_{j}' in machine_node['dynamic_edges']['executing'] else 0 for j in range(n_jobs)] +
            [1 if f'J_{j}' in machine_node['dynamic_edges']['waiting'] else 0 for j in range(n_jobs)] +
            [1 if f'J_{j}' in machine_node['dynamic_edges']['routing'] else 0 for j in range(n_jobs)]
        )
        return features
    ...
    def get_job_features(self, job_id):
        ...
        features = (
            [1 if f'M_{m}' in job_node['dynamic_edges']['executing'] else 0 for m in range(n_machines)] +
            [1 if f'M_{m}' in job_node['dynamic_edges']['waiting'] else 0 for m in range(n_machines)] +
            [1 if f'M_{m}' in job_node['dynamic_edges']['routing'] else 0 for m in range(n_machines)]
        )
        return features
