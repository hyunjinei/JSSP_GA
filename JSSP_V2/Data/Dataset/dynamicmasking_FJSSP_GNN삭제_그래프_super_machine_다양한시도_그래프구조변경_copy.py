...
class MultiAgentSystem:
    ...
    def get_supervisor_features(self, state):
        ...
        print(f"Number of machine features: {len(machine_features)}")
        print(f"Number of job features: {len(job_features)}")
        print(f"Number of environment features: {len(env_features)}")
        print(f"Total number of supervisor features: {len(features)}")        
...
def predict(multi_agent_system, env, test_dataset, num_predictions=1, max_steps=100000000):
    ...
    while valid_solution_count < num_predictions and step_count < max_steps:
        ...
        episode_reward = 0
        multi_agent_system.reset(reset_epsilon=True)
