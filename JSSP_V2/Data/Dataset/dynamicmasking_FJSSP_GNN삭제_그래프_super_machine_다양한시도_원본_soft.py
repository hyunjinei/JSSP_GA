...
class DDQNAgent:
    ...
    def act(self, state, valid_operations):
        ...
        # 소프트 엡실론 그리디 전략 적용
        exp_q_values = np.exp(q_values.detach().numpy()[valid_q_indices])  # valid_q_indices에 해당하는 Q 값에 대한 소프트맥스
        probabilities = exp_q_values / np.sum(exp_q_values)  # 확률로 변환
...
class SupervisorAgent(DDQNAgent):
    ...
    def select_machine(self, available_machines, state_features):
        ...
        q_sum = sum(np.exp(list(valid_q_values.values())))

        if q_sum == 0:
            probabilities = [1.0 / len(valid_q_values)] * len(valid_q_values)
        else:
            probabilities = [np.exp(q) / q_sum for q in valid_q_values.values()]
