...
class DDQNAgent:
    ...
    def update_epsilon(self, global_episode_count):
        if self.epsilon > self.epsilon_min:
            self.epsilon *= self.epsilon_decay# ** global_episode_count
        # print(f"Agent epsilon updated to: {self.epsilon}")

    def replay(self):
        if self.memory.n_entries < self.replay_start_size:
            return
        # print(f"Agent replay called")  # Add this line to check when replay is called
...
class SupervisorAgent(DDQNAgent):
    ...
    def select_machine(self, available_machines, state_features):
        ...
        best_action = current_q_values.argmax().item()
    ...
    # def replay(self):
    #     if self.memory.n_entries >= self.replay_start_size:
    #         print("Supervisor replay called")  # Add this line
    #         super().replay()
...
class MultiAgentSystem:
    ...
    def end_episode(self, state_features, reward_task, reward_machine, done):
        ...
        print(f"Global episode count: {self.global_episode_count}")  # 디버그 출력
...
def predict(multi_agent_system, env, test_dataset, num_predictions=1, max_steps=100000000, epsilon=0.0001):
    ...
    all_predictions = []
    step_count = 0
    # multi_agent_system.reset()  # 초기화는 예측 모드 설정 전에 수행
...
#abz5
    ...
    all_predictions = predict(multi_agent_system, env, test_dataset, num_predictions=230, max_steps=100000000)
