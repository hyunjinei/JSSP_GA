...
class CommNetAgent(DQNAgent):
    def __init__(self, max_state_size, action_size, gnn, update_target_frequency=32, replay_start_size=1000):
        super(CommNetAgent, self).__init__(max_state_size, action_size, update_target_frequency, replay_start_size)
        self.gnn = gnn
        self.comm_var = None
        self.hidden_state = None

    def update_comm_var(self, other_agent_states):
        # 상대방의 상태가 None인 경우 빈 배열로 처리하여 평균을 계산할 수 있게 합니다.
        if other_agent_states and other_agent_states[0] is not None:
            self.comm_var = np.mean(other_agent_states, axis=0)
        else:
            self.comm_var = None

    def update_hidden_state(self, state, gnn_output):
        if self.hidden_state is None:
            self.hidden_state = np.zeros_like(state)

        combined_state = np.concatenate((state, gnn_output))
        if self.comm_var is not None:
            combined_state = np.concatenate((combined_state, self.comm_var))

        self.hidden_state = np.tanh(combined_state)  # 예를 들어, tanh 활성화 함수를 사용합니다.

    def act(self, state, mask=None, env=None):
        graph_data = create_graph(env)
        gnn_output = self.gnn(graph_data).flatten().detach().numpy()

        self.update_hidden_state(state, gnn_output)
        padded_state, mask = pad_state_action(self.hidden_state, [])

        return super(CommNetAgent, self).act(padded_state, mask)

class TaskAgent(CommNetAgent):
    def __init__(self, max_state_size, action_size, gnn, update_target_frequency=32, replay_start_size=1000):
        super(TaskAgent, self).__init__(max_state_size, action_size, gnn, update_target_frequency, replay_start_size)

class MachineAgent(CommNetAgent):
    def __init__(self, max_state_size, action_size, gnn, update_target_frequency=32, replay_start_size=1000):
        super(MachineAgent, self).__init__(max_state_size, action_size, gnn, update_target_frequency, replay_start_size)
...
def train_individual_models(datasets, num_episodes_per_dataset):
    ...
    for i, dataset in enumerate(datasets):
        ...
        for episode in range(num_episodes_per_dataset):
            ...
            episode_reward = 0
            while not done:
                # 각 스텝에서 상대 에이전트의 상태를 업데이트
                task_agent.update_comm_var([machine_agent.hidden_state if machine_agent.hidden_state is not None else np.zeros_like(raw_state)])  # task_agent가 machine_agent의 상태를 받아 업데이트
                machine_agent.update_comm_var([task_agent.hidden_state if task_agent.hidden_state is not None else np.zeros_like(raw_state)])  # machine_agent가 task_agent의 상태를 받아 업데이트

                # TaskAgent의 행동 결정
                task_action = task_agent.act(raw_state, None, env)
                ...
                # MachineAgent의 행동 결정
                machine_action = machine_agent.act(raw_state, None, env)
                ...
                # 메모리에 상태와 보상 저장
                task_agent.remember(raw_state, selected_job * env.n_machines + selected_op, step_reward, raw_next_state, done)
                machine_agent.remember(raw_state, selected_op * env.n_jobs + selected_job, step_reward, raw_next_state, done)
...
def predict(task_agent, machine_agent, env, test_dataset, num_predictions=1, max_steps=100000000):
    ...
    while valid_solution_count < num_predictions and step_count < max_steps:
        ...
        state, mask = pad_state_action(raw_state, [], max_state_size=1800)
        ...
        while not done and step_count < max_steps:
            ...
            task_agent.update_comm_var([raw_state])
            machine_agent.update_comm_var([raw_state])
