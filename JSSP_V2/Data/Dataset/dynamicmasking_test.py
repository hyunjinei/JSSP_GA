...
def train_individual_models(datasets):
    ...
    for i, dataset in enumerate(datasets):
        ...
        for episode in range(1000):  # 원하는 에피소드 수 설정
            state = env.reset()
            ...
            while not done:
                ...
                if done:
                    ...
                    print(f"Training: Episode {episode+1} processed. Reward Task: {reward_task}, Reward Machine: {reward_machine}")
                    break
            ...
            if episode % 100 == 0:
                logging.info(f"Episode {episode}/{1000} completed for dataset {dataset}")
                print(f"Episode {episode}/{1000} completed for dataset {dataset}")
...
def main():
    ...
    all_predictions = predict(combined_task_agent, combined_machine_agent, env, test_dataset, num_predictions=100, max_steps=100000000)
