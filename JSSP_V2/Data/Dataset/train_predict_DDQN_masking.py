...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        self.target_model2 = DQN(state_size, action_size)
    ...
    def replay(self, batch_size):
        ...
        for idx, (state, action, reward, next_state, done) in enumerate(minibatch):
            ...
            if not done:
                ...
                target += self.gamma * self.target_model2(next_state)[0][best_action].item()
            target_f = self.model(state)
            mask = (state != -1).float()
            target_f[0][action] = target * mask[0][action]
...
def train_individual_models(datasets):
    ...
    for i, dataset in enumerate(datasets):
        ...
        repeat_count = 20 if len(solutions) < 100000 else len(solutions)
    ...
    return models, state_size, action_size
