...
# def predict(agent, env, test_dataset, max_steps=1000000, num_predictions=10000):
    all_solutions = []
    for _ in range(num_predictions):  # 예측을 여러 번 수행하여 다양한 솔루션을 생성
        state = env.reset()
        state = np.concatenate((np.array([state[0]]), state[1], state[2]))
        solution = []
        done = False
        ...
        if len(solution) == total_operations:
            ...
            print("Not all operations are used in the solution. Skipping this solution.")
    
    if not all_solutions:
        raise ValueError("No valid solutions found. Please check the prediction logic.")
    
    return all_solutions

def predict(agent, env, test_dataset, num_predictions=1000, max_steps=100000):
...
def main():
    datasets = ['la17.txt', 'orb08.txt', 'la16.txt', 'la01.txt', 'orb09.txt', 'la02.txt', 'la19.txt', 'la20.txt', 'la18.txt', 'orb10.txt', 'la03.txt', 'la04.txt','abz5.txt', 'abz6.txt', 'dmu32.txt', 'dmu33.txt', 'dmu35.txt', 'ft06.txt', 'ft10.txt', 'ft20.txt', 'la05.txt', 'la06.txt', 'la07.txt', 'la08.txt', 'la09.txt', 'la10.txt', 'la11.txt', 'la12.txt','la13.txt', 'la14.txt', 'la15.txt', 'la23.txt', 'la26.txt', 'la28.txt', 'la30.txt','la31.txt', 'la32.txt', 'la33.txt', 'la34.txt', 'la35.txt', 'orb01.txt', 'orb02.txt', 'orb03.txt', 'orb04.txt', 'orb05.txt', 'orb06.txt', 'orb07.txt',  'swv16.txt', 'swv17.txt', 'swv18.txt', 'swv19.txt', 'swv20.txt', 'ta01.txt']
    models = train_individual_models(datasets)
    test_dataset = RLDataset('la02.txt')
    ...
    all_predictions = predict(combined_model, env, test_dataset, num_predictions=100000000000, max_steps=10000000000000)
    logging.info(f"Predicted Solutions: {all_predictions}")
    print("Predicted Solutions:", all_predictions)

    # 여러 솔루션 중 가장 좋은 솔루션 선택
    best_solution = None
