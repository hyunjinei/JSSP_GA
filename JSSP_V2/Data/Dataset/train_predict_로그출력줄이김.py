...
# pretrain_with_solutions 수정본
def pretrain_with_solutions(agent, solutions, env, repeat_count=100000):
...
#         if dataset in ['orb08.txt', 'orb09.txt', 'la17.txt', 'la01.txt']:
...
# train_individual_models 수정 후(pretrain_with_solutions 수정으로 인해)
def train_individual_models(datasets):
    models = [] 
    for i, dataset in enumerate(datasets):
        logging.info(f"Starting training for dataset {dataset} ({i+1}/{len(datasets)})")
        ...
        # 솔루션 개수가 100,000개보다 적은 경우 repeat_count 설정
        ...
        # replay 메서드를 통해 학습 진행
        for _ in range(100):  # 예시로 100번 반복 학습
            agent.replay(64)  # 배치 크기 64로 설정
...
def predict(agent, env, test_dataset, num_predictions=2000, max_steps=100000000):
    ...
    while valid_solution_count < num_predictions and step_count < max_steps:
        ...
        while not done and step_count < max_steps:
            ...
            if step_count % 1000 == 0:  # 로그 빈도를 줄임
        ...
        if len(solution) == total_operations:
            ...
            if valid_solution_count % 1000 == 0:  # 로그 빈도를 줄임
...
def main():
    datasets = [ 'swv16.txt', 'swv17.txt', 'swv18.txt', 'swv19.txt', 'swv20.txt', 'ta01.txt']
    ...
    all_predictions = predict(combined_model, env, test_dataset, num_predictions=2000, max_steps=100000000)
