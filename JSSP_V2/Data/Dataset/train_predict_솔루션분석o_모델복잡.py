...
def pretrain_with_solutions(agent, solutions, env, num_epochs=100):
...
def predict(agent, env, test_dataset, max_steps=1000000):
    state = env.reset()
    state = np.concatenate((np.array([state[0]]), state[1], state[2]))
    solution = []
    done = False
...
def main():
    datasets = ['la02.txt', 'abz5.txt', 'abz6.txt', 'ft10.txt']
