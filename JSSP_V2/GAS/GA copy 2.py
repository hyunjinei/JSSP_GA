...
def migrate_top_10_percent(ga_engines, migration_order, island_mode):
    ...
    for source_island_idx in range(num_islands):
        ...
        for best_individual in top_10_percent:
            ...
            # print(f"  Sequence: {best_individual.seq}, Fitness: {best_individual.fitness}, Makespan: {best_individual.makespan}")
            ...
            # print(f"  Sequence: {target_island.population.individuals[replacement_idx].seq}, Fitness: {target_island.population.individuals[replacement_idx].fitness}, Makespan: {target_island.population.individuals[replacement_idx].makespan}")
        ...
        try:
            ...
            for generation in range(self.config.generations):
                ...
                if self.migration_frequency and (generation + 1) % self.migration_frequency == 0 and self.ga_engines:
                    print(f"GA {self.ga_engines.index(self)+1} - Preparing for migration at generation {generation + 1}")
                    if self.island_mode == '2':
                        print(f"GA {self.ga_engines.index(self)+1} - Migration 중 (순차) at generation {generation + 1}")
                        ...
                        print(f"GA {self.ga_engines.index(self)+1} - Migration 중 (랜덤) at generation {generation + 1}")
                    ...
                    for method in self.local_search_methods:
                        for i in range(len(top_individuals)):
                            ...
                            if optimized_ind in self.population.individuals:
                                ...
                                print("Added optimized individual to population.")
