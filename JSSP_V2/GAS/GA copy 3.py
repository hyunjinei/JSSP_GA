    ...
    def evolve(self, index, sync_generation, sync_lock, events=None):
        try:
            ...
            while sync_generation[index] < self.config.generations:
                ...
                print(f"Initial population size: {len(self.population.individuals)}")
                print(f"First individual sequence: {self.population.individuals[0].seq}")
                print(f"First individual makespan: {self.population.individuals[0].evaluate()}")
                for individual in self.population.individuals:
                    individual.evaluate()
                    individual.calculate_fitness(self.config.target_makespan)

                self.population.individuals.sort(key=lambda x: x.makespan)
                ...
                if current_best.makespan < best_fitness:
                    ...
                    best_individual.create_monitor()  # 여기에 추가
                    print(f"GA{index+1}_Best makespan at generation {sync_generation[index]}: {best_fitness}")
                    print(f"GA{index+1}_Best individual sequence: {best_individual.seq}")

                print(f"GA{index+1} Population: {self.population is not None}")
                print(f"GA{index+1} Best Individual: {current_best is not None}")
                ...
                if sync_generation[index] > 0 and sync_generation[index] % self.local_search_frequency == 0:
                    ...
                    top_individuals = self.population.individuals[:int(len(self.population.individuals) * self.local_search_top_percentage)]
                ...
                if sync_generation[index] > 0 and self.selective_mutation and sync_generation[index] % self.selective_mutation_frequency == 0:
                    print(f'GA{index+1}_Selective Mutation 적용')
                ...
                if current_best.makespan < best_makespan:
                    ...
                    print(f"GA{index+1}_Best makespan at generation {sync_generation[index]}: {best_makespan}")
                    print(f"GA{index+1}_Best individual sequence: {best_individual.seq}")
            ...
            if best_individual is not None:
                if best_individual.monitor is not None:
                    best_individual.monitor.save_event_tracer(self.config.filename['log'])
                else:
                    print("Best individual has no valid monitor to save the event tracer.")
            else:
                print("No valid best individual found.")
