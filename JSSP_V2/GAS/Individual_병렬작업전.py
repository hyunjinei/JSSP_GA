...
class Individual:
    ...
    def calculate_fitness(self):
        ...
        # max_makespan = 10000  # Adjust according to problem scale
        # self.fitness = 1 / (1 + self.makespan / max_makespan)        
        # self.fitness = math.exp(-self.makespan / 100)  # Exponential fitness function        
        # self.fitness = 1 / self.makespan
        # print(f"Calculated fitness: {self.fitness}, Makespan: {self.makespan}")
        return self.fitness

        # for job_id in range(self.config.n_job):
        #     job_operations = [(op[0], op[1], op[2], op[3], op[4]) for op in zip(self.monitor.time, self.monitor.event, self.monitor.part, self.monitor.process_name, self.monitor.machine_name) if op[2] == f'Part{job_id}_0']
        #     print(f"Job {job_id} operations: {job_operations}")
