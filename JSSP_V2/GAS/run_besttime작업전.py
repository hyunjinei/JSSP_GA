...
# GA 엔진 실행 함수
def run_ga_engine(ga_engine, index):
    try:
        best, best_crossover, best_mutation, all_generations, execution_time = ga_engine.evolve()
        if best is None:
            return None
        return best, best_crossover, best_mutation, all_generations, execution_time
...
def main():
    ...
    config = Run_Config(n_job=3, n_machine=3, n_op=9, population_size=50, generations=4, 
                ...
                while best_individuals[index] is None:
                    try:
                        ...
                        if result is None:
                            ...
                            best, best_crossover, best_mutation, all_generations, execution_time = result
                            best_individuals[index] = (best, best_crossover, best_mutation, execution_time, all_generations)
    ...
    for i, result in enumerate(best_individuals):
        if result is not None:
            best, best_crossover, best_mutation, execution_time, all_generations = result
            crossover_name = best_crossover.__class__.__name__
            mutation_name = best_mutation.__class__.__name__
