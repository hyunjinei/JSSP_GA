...
class Individual:
    def __init__(self, config=None, seq=None, solution_seq=None, op_data=None):
        ...
        self.makespan, self.mio_score, self.idle_time, self.real_time = self.evaluate(self.machine_order)
        self.score = calculate_score(self.MIO, self.MIO_sorted)
        self.machines = [Machine(simpy.Environment(), i) for i in range(config.n_machine)]
        self.calculate_fitness(config.target_makespan, idle_time=self.idle_time, multi_objective=True)
    ...
    def calculate_fitness(self, target_makespan, idle_time=None, multi_objective=False):
        if self.real_time == 0:
            raise ValueError("Real time is zero, which will cause division by zero error.")
        
        if not multi_objective:
            self.fitness = 1 / (self.makespan / target_makespan)
            print(f"[Single Objective] fitness: {self.fitness}, makespan: {self.makespan}, target_makespan: {target_makespan}")
        else:
            combined_time = self.real_time + idle_time
            self.fitness = 1 / (combined_time / target_makespan)
            print(f"[Multi Objective] fitness: {self.fitness}, combined_time: {combined_time}, real_time: {self.real_time}, idle_time: {idle_time}, target_makespan: {target_makespan}")
    ...
    def evaluate(self, machine_order):
        ...
        for i in range(self.config.n_machine):
            mio = [task["op_id"] for task in model['M' + str(i)].tasks]
            self.MIO.append(mio)
        ...
        for machine in model.values():
            if isinstance(machine, Machine):
                idle_time, makespan, real_time = machine.calculate_idle_time_and_makespan()
                total_idle_time += idle_time
                total_real_time += real_time
                if makespan > max_makespan:
                    max_makespan = makespan
                print(f"Machine {machine.idx}: idle_time = {idle_time}, makespan_machine = {makespan}, real_time = {real_time}")

        self.real_time = total_real_time
        self.idle_time = total_idle_time

        return max_makespan, mio_score, total_idle_time, total_real_time
